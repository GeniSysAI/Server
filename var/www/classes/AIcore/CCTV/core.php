<?php 
    ...
    class TASS
    {
        ...
        {   
            ...
            ");
            $pdoQuery->execute([
                ':tassID' => $this->_GeniSys->_helpers->encrypt(filter_input(INPUT_POST, 'deviceID', FILTER_SANITIZE_NUMBER_INT))
            ]);
        ...
        {   
            if(!filter_input(INPUT_POST, 'tassID', FILTER_SANITIZE_NUMBER_INT)):
                return [
                    "Response"=>"FAILED",
            ...
            if(!filter_input(INPUT_POST, 'tassAddress', FILTER_SANITIZE_STRING)):
                return [
                    "Response"=>"FAILED",
            ...
            ");
            $pdoQuery->execute([
                ':tassID' => $this->_GeniSys->_helpers->encrypt(filter_input(INPUT_POST, 'tassID', FILTER_SANITIZE_NUMBER_INT)),
                ':tassAddress' => $this->_GeniSys->_helpers->encrypt(filter_input(INPUT_POST, 'tassAddress', FILTER_SANITIZE_STRING))
            ]);
            $pdoQuery->closeCursor();
            $pdoQuery = null;
            
            if(filter_input(INPUT_POST, 'tassID', FILTER_SANITIZE_NUMBER_INT) != filter_input(INPUT_POST, 'deviceIDOld', FILTER_SANITIZE_NUMBER_INT)):
                    return [
                        "Response"=>"OK",
                        "Redirect"=>"/TASS/"
                    ];
